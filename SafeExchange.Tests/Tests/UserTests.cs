...
namespace SafeExchange.Tests
{
    ...
    public class UserTests
    {
        ...
        public async Task RotationRace_DuplicateRetiredIdMapEntry_IsSwallowed_RequestSucceeds()
        {
            ...
            // The racing request rotated the user on a separate DbContext, so the shared
            // fixture context now tracks a stale etag. Clear it so TearDown re-reads current
            // etags instead of failing its delete with a 412 precondition mismatch.
