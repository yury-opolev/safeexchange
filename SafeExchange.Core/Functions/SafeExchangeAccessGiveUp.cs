    ...
    public class SafeExchangeAccessGiveUp
    {
            ...
            ILogger log)
        {
            ...
            return request.Method.ToLower() switch
            {
                ...
                "delete" => await this.GiveUpAsync(request, existingMetadata, subjectType, subjectId, log),
            ...
            => await ActionResults.TryCatchAsync(request, async () =>
        {
            ...
            // Simulate the caller's row being removed when assessing custodianship —
            // a sole custodian asking the preview question expects WouldOrphan=true.
            var preview = await this.orphanedSecretManager.PreviewAsync(secretId, this.dbContext, subjectType, subjectId);
        ...
        private async Task<HttpResponseData> GiveUpAsync(
            HttpRequestData request, ObjectMetadata secret, SubjectType subjectType, string subjectId, ILogger log)
            => await ActionResults.TryCatchAsync(request, async () =>
        {
                ...
                await this.auditWriter.AppendAsync(
                    ...
                    new
                    {
                        target = new
                        {
                            ...
                            subjectId = subjectId,
                            subjectName = directRow.SubjectName,
                        },
                        permissions = new
                        {
                            from = AuditPayloads.PermissionFlags(beforePerm),
                            to = AuditPayloads.PermissionFlags(PermissionType.None),
                        },
                        reason = "give-up",
                    }, log);
            }
