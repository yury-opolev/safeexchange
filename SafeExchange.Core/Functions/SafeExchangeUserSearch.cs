...
namespace SafeExchange.Core.Functions
{
    ...
    public class SafeExchangeUserSearch
    {
            ...
            HttpRequestData request, ClaimsPrincipal principal, ILogger log)
        {
            ...
            log.LogInformation($"{nameof(SafeExchangeUserSearch)} triggered by {subjectType} (tid {TelemetryContext.Current}), [{request.Method}].");
            ...
            => await ActionResults.TryCatchAsync(request, async () =>
            {
                if (!this.features.UseGraphUserSearch)
                {
                    return await ActionResults.CreateResponseAsync(
                        ...
                        {
                            Status = "no_content",
                            Result = Array.Empty<GraphUserOutput>().ToList()
                        });
                }
