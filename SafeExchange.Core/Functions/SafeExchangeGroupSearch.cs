...
namespace SafeExchange.Core.Functions
{
    ...
    public class SafeExchangeGroupSearch
    {
            ...
            HttpRequestData request, ClaimsPrincipal principal, ILogger log)
        {
            ...
            log.LogInformation($"{nameof(SafeExchangeGroupSearch)} triggered by {subjectType} (tid {TelemetryContext.Current}), [{request.Method}].");
        ...
        private async Task<HttpResponseData> HandleSearchGroup(HttpRequestData request, ClaimsPrincipal principal, ILogger log)
            => await ActionResults.TryCatchAsync(request, async () =>
            {
                ...
                {
                    log.LogWarning("Rejected group-search input: {Reason}", validationError);
                    return await ActionResults.CreateResponseAsync(
                        request, HttpStatusCode.BadRequest,
